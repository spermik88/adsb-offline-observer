...
- Repository source decision: `external/Dump1090` may stay as vendored upstream source for reference, but release artifacts are taken only from `BundledAssets/backend/dump1090`
...
- Current publish output already ships without extra localized runtime folders, so no additional trimming step is required right now
